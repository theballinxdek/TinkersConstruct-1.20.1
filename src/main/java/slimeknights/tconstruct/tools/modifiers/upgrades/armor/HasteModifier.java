...
public class HasteModifier extends Modifier implements ToolStatsModifierHook, TooltipModifierHook {
  ...
  public void addToolStats(IToolContext context, ModifierEntry modifier, ModifierStatsBuilder builder) {
    ToolStats.MINING_SPEED.add(builder, 4 * modifier.getEffectiveLevel());
  }

  @Override
  public void addTooltip(IToolStackView tool, ModifierEntry modifier, @Nullable Player player, List<Component> tooltip, TooltipKey tooltipKey, TooltipFlag tooltipFlag) {
    if (tool.hasTag(TinkerTags.Items.ARMOR)) {
      double boost = 0.1 * modifier.getEffectiveLevel();
      if (boost != 0) {
